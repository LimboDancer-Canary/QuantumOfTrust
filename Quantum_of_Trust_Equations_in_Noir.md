...
> "A zero-knowledge proof is generated by combining a statement (what you want to prove) with a witness (the private data that makes the statement true). The verifier can check that the proof is valid without ever seeing the witness."

**First equation**: The prover creates a proof from two inputs:
- **Statement**: The claim being proven (e.g., "My trust ≥ 50")
- **Witness**: The private evidence (e.g., your full contract history)

**Second equation**: The verifier checks the proof and gets either true or false — nothing else. They learn whether the statement is true, but nothing about *why* it's true.

**Real-world analogy**: It's like proving you're over 21 to enter a bar without showing your actual birthdate — they learn "yes, over 21" but not your age or ID number.
...
> "A signed number is represented as a pair: its absolute value (magnitude) and whether it's positive or negative (sign). The magnitude lives in the finite field (always a positive integer), while the sign is tracked separately as a boolean flag."

This is an **implementation technique**, not a conceptual QoT equation. Noir's `Field` type only supports positive integers (elements of a prime field), but trust values can be negative (actively distrusted agents). We solve this by storing the absolute value and sign separately, then implementing arithmetic operations that respect the sign.

**Real-world analogy**: It's like writing "-50" as "50, negative" — you store the number part and the minus sign in different places.
...
> "To represent decimal numbers in integer-only arithmetic, we multiply by a scale factor (one million). To convert back to the real value, divide by the scale factor."

This is another **implementation technique**. ZK circuits work with integers only, but we need decimal precision for weights like 0.75 or trust values like 3.14159. We store `3.14159` as `3141590` (the integer `3.14159 × 1,000,000`), perform all math on these scaled integers, then interpret the results by mentally dividing by the scale factor.

**Example**: To compute `0.5 × 0.5 = 0.25`:
- Store 0.5 as 500,000
- Multiply: 500,000 × 500,000 = 250,000,000,000
- Divide by SCALE: 250,000,000,000 / 1,000,000 = 250,000
- Result 250,000 represents 0.25 ✓
...
> "A Quantum of Trust is defined as one of two things: either it's an **Agent** (which has a skill type `t` and a history of contracts `h_t` in that skill), or it's a **DAO** (which is a group containing other Quanta of Trust)."

This is the foundational recursive type definition:
- **Agent(t, h_t)**: A single Avatar with a specific skill and track record
- **DAO({q⟨T⟩})**: A group that contains other q⟨T⟩ units (which can themselves be Agents or DAOs)

The `::=` symbol means "is defined as," and the `|` symbol means "or."

**Real-world analogy**: It's like saying "A LEGO structure is either a single brick, or a collection of LEGO structures." You can build unlimited organizational complexity from this simple definition.
...
> "The trust valuation function V takes any Quantum of Trust and produces a real number. The subscript `t` indicates this is scoped to a specific skill type."

**What the trust value means**:
- **Zero**: No track record yet — a newcomer with unknown reliability
- **Positive**: More successes than failures — this entity has demonstrated trustworthiness
- **Negative**: More failures than successes — this entity has demonstrated *un*trustworthiness (worse than unknown)
...
> "An Agent's trust value equals the sum of all their contract contributions. Each contract contributes its weight (ω) multiplied by its outcome."

Breaking it down:
- **Σ (sum)**: Add up contributions from every contract in the history
- **c ∈ h_t**: For each contract `c` in the agent's history `h_t` for skill type `t`
- **ω(c)**: The weight of the contract (how much signal it provides)
- **outcome(c)**: The result of the contract (+1 for success, -1 for failure, values in between for partial outcomes)

**Example**: An engineer with three contracts:
1. Weight 2.0 × outcome +1.0 = +2.0
2. Weight 1.5 × outcome +0.5 = +0.75
3. Weight 1.0 × outcome -1.0 = -1.0

**Total trust = 2.0 + 0.75 - 1.0 = 1.75**
...
> "A DAO's trust value is computed by applying an aggregation function Φ to the trust values of all its members."

The aggregation function Φ can be:
- **Sum**: Total capability — the DAO can handle work equal to combined member capacity
- **Average**: Mean reliability — expected reliability of a randomly selected member
- **Minimum**: Weakest-link — the DAO is only as strong as its weakest member (for security-critical work)
- **Maximum**: Best-member — capability defined by the strongest member

**Example**: A 3-person DAO with member trust values [10, 20, 30]:
- Sum: 60 (total capacity)
- Average: 20 (typical member)
- Minimum: 10 (security-critical interpretation)
- Maximum: 30 (best-case capability)
...
> "A contract is defined by six elements: who's providing the service, who's consuming it, what skill type it involves, how much is at stake, how difficult it is, and its deadline."

**The contract tuple**:
- **a_provider**: The Avatar doing the work
- **a_consumer**: The Avatar commissioning the work (customer)
- **t**: The skill type (e.g., "engineering", "design")
- **s**: The stake (token amount at risk)
- **d**: The difficulty rating (0–10 scale)
- **τ (tau)**: The deadline/completion timestamp

**Note**: The outcome (success/failure) is determined after completion and stored separately. The contract definition captures what was agreed *before* work began.
...
> "The outcome of a contract is a value between -1 (complete failure) and +1 (complete success). Values in between represent partial outcomes."

**The outcome range**:
- **+1**: Complete success — all deliverables met, customer fully satisfied
- **0**: Neutral — partial delivery, mixed results
- **-1**: Complete failure — nothing delivered, total breach

**Why continuous instead of discrete?** Real-world outcomes are nuanced. A project might be delivered late but with excellent quality, or on time but with minor defects. The continuous range captures this nuance.

**Discrete special case**: For simpler evaluation, {-1, 0, +1} can represent {failure, partial, success}.

**In Noir**: Since fields are always positive, we store outcomes using offset encoding: `stored = outcome + 100`, giving range [0, 200] representing [-100, +100].
...
> "The weight of a contract is a function of four factors: stake, difficulty, counterparty trust, and recency."

Not all contracts are created equal. Completing a difficult, high-stakes contract for a reputable counterparty should contribute more to your reputation than completing an easy, low-value contract for an unknown entity.

**The four factors**:
1. **Stake s(c)**: Higher-value contracts carry more signal (logarithmic scaling)
2. **Difficulty d(c)**: Harder work carries more signal (linear scaling, range 0.5–2.0×)
3. **Counterparty trust V_t**: Contracts with trusted counterparties carry more signal (sigmoid bounding, range 0.5–1.5×)
4. **Recency**: Recent contracts weighted more heavily than old ones (exponential decay, half-life 365 days)

**Example**: A $10,000 difficulty-8 contract completed yesterday for a trusted client might have weight 15.0, while a $100 difficulty-2 contract from 2 years ago for an unknown client might have weight 0.3.
...
> "An agent's history at step n+1 equals their history at step n plus the newly completed contract. History only grows — it's append-only."

This equation describes how contract history evolves over time:
- **h_t^(n)(a)**: Agent `a`'s history at step `n` for skill type `t`
- **∪ {c_n}**: Union with the new contract `c_n` (set addition)
- **h_t^(n+1)(a)**: The updated history

**Key properties**:
- History is immutable — you can't remove or modify past contracts
- Each skill type has its own independent history
- On-chain, this history is tamper-proof and publicly verifiable

**Real-world analogy**: It's like a permanent employment record. You can add new entries, but you can't erase old ones.
...
> "An agent's trust value at step n+1 equals their trust at step n plus the contribution from the new contract."

This is the **incremental update** formula. Instead of recomputing trust from the entire history every time (expensive), we simply add the new contract's contribution to the existing trust value.

**The contribution** = weight × outcome:
- Successful contract (outcome > 0): Trust increases
- Failed contract (outcome < 0): Trust decreases
- Higher weight: Larger change in either direction

**Example**:
- Current trust: 5.0
- New contract: weight 2.0, outcome +0.5 (partial success)
- Contribution: 2.0 × 0.5 = 1.0
- New trust: 5.0 + 1.0 = 6.0
...
> "An agent is eligible for a contract if and only if their trust value meets or exceeds the contract's threshold. The threshold is calculated from the contract's stake and difficulty."

**The eligibility condition**: Trust ≥ Threshold

**The threshold formula**:
- **log(1 + stake)**: Logarithmic scaling of stake (diminishing returns for very high stakes)
- **× difficulty**: Linear scaling by difficulty (0–10)

**Why this formula?**
- Higher stakes require higher trust (you need more credibility to handle more money)
- Higher difficulty requires higher trust (harder work needs proven capability)
- The logarithm prevents extremely high stakes from requiring impossibly high trust

**Example thresholds**:
- $100 stake, difficulty 5: log(101) × 5 ≈ 23
- $10,000 stake, difficulty 5: log(10001) × 5 ≈ 46
- $10,000 stake, difficulty 10: log(10001) × 10 ≈ 92
...
> "As the number of contracts approaches infinity, the correlation between computed trust values and actual real-world reliability approaches perfect correlation (1.0)."

This is the **validation criterion** for the trust system:
- **V_t^(n)(a)**: The computed trust value after `n` contracts
- **R_t(a)**: The agent's *actual* real-world reliability (unobservable ground truth)
- **Corr()**: Statistical correlation (-1 to +1)
- **lim n→∞**: As history grows without bound

**What this means**: If the system is working correctly, agents with high computed trust values should actually be more reliable in practice, and this relationship should strengthen as more data accumulates.

**Why it matters**: This is how we validate that the mathematical framework captures something real. If V_t didn't correlate with actual reliability, the system would be useless.

**In ZK circuits**: This is validated off-circuit through network-wide statistical analysis, not proven in individual eligibility proofs.
...
> "The size of an honest agent's history is greater than the size of any individual Sybil identity's history. This holds for all Sybil identities `i`."

**The Sybil attack**: An attacker creates multiple fake identities to game the reputation system.

**Why this inequality defeats Sybil attacks**: If an attacker splits their activity across N fake identities, each identity has only ~1/N of the history they would have with a single honest identity. Since many eligibility checks require minimum history size, the attacker would need to complete N times as many contracts to meet the same thresholds — making the attack economically irrational.

**Example**: 
- Honest Alice completes 100 contracts → history size = 100
- Sybil attacker Bob splits activity across 10 identities → each identity has ~10 contracts
- If contracts require minimum 50 history size, Alice qualifies but Bob's identities don't
...
> "The prove_eligibility function takes a private history, skill type, and threshold, and produces a proof π. This proof demonstrates that the agent's trust value meets the threshold WITHOUT revealing the underlying history."

**Inputs**:
- **h_t**: The agent's contract history (PRIVATE — verifier never sees this)
- **t**: The skill type being proven (public)
- **θ**: The threshold to meet (public)

**Output**:
- **π**: A cryptographic proof (verifier can check this is valid)

**What the verifier learns**: "This agent is eligible" (true/false)

**What the verifier does NOT learn**:
- The agent's exact trust score
- How many contracts are in the history
- Who the counterparties were
- What the individual outcomes were
- How close or far above the threshold they are
...
> "Customer skill types are six dimensions that measure how good someone is as a *customer*, separate from how good they are as a *provider*. Each dimension captures a different aspect of customer behavior."

**The six customer skill types**:

1. **Commitment**: Do they finish what they start? (projects completed / projects initiated)
2. **Escrow Discipline**: Do they pay on time and release funds promptly?
3. **Verification Integrity**: Do they rate fairly? (not rubber-stamping, not erratic)
4. **Scope Stability**: Do they stick to agreed specifications? (tasks as planned / total tasks)
5. **Timeline Realism**: Do they set achievable deadlines? (accuracy of estimates)
6. **Spec Quality**: Do their specs lead to successful implementations?

**Why orthogonal to provider skills?** Someone can be an excellent engineer (high provider trust in "engineering") but a terrible customer (abandons projects, late payments, unfair ratings). These are independent dimensions.

**Implementation note**: Customer skill types use a reserved ID range (1000001–1000006) to avoid collision with provider skill type hashes.
...
> "A customer's trust value is the sum of their behavioral contributions across all contracts, weighted by contract importance and adjusted for Sybil resistance factors."

**Key difference from provider trust**: Providers are judged by *outcomes assigned by customers* (did the work succeed?). Customers are judged by *observable behaviors* (did they pay on time? rate fairly? provide clear specs?).

**The formula components**:
- **Σ**: Sum across all contracts where this entity was the customer
- **ω(c)**: Contract weight (same as provider weighting)
- **behavior(c)**: The customer's behavior metric for this contract (e.g., funding timeliness, rating fairness)
- **γ(c)**: Counterparty trust factor (Sybil resistance)
- **ν(c)**: Velocity weight (Sybil resistance)

**Why measure customer trust?** Bad customers harm the ecosystem: late payments, unfair ratings, scope creep, unrealistic deadlines. Measuring customer trust enables bidirectional accountability — providers can check if a potential customer is reliable before accepting work.
...
> "The verification weight is a function of the customer's verification integrity score and their rating variance. It determines how much the customer's rating should influence the provider's trust score."

**Range**: [0.5, 1.5] — credible customers' ratings count up to 3× more than unreliable customers'

**The two factors**:
1. **V_verify (verification integrity)**: Has this customer demonstrated fair, thoughtful rating behavior?
2. **σ² (rating variance)**: How varied are this customer's historical ratings?

**Why rating variance matters**:
- **Too low (σ² < 0.1)**: Customer gives everyone the same rating = "rubber-stamping" (uninformative)
- **Too high (σ² > 0.8)**: Customer's ratings are wildly inconsistent = "erratic" (unreliable)
- **Just right (~0.35)**: Customer discriminates between good and bad work = credible signal

**Example**: If Alice (credible, weight 1.4) and Bob (rubber-stamper, weight 0.6) both rate you +1.0:
- Alice's contribution: 1.4 × 1.0 = 1.4
- Bob's contribution: 0.6 × 1.0 = 0.6

This makes rubber-stamping and erratic rating self-penalizing without external governance.
...
**First equation** (task stake allocation):
> "A task's stake equals the phase stake multiplied by the task's proportional weight. The customer assigns weights during Planning; stake is derived from those weights."

**Second equation** (phase difficulty aggregation):
> "A phase's difficulty is the stake-weighted average of its task difficulties. The customer controls stake (via weights); the provider assesses difficulty. Both influence the aggregate."

### The Core Problem: Information Asymmetry

Accurately assessing task difficulty requires domain expertise. The customer typically lacks this expertise—that's why they're hiring a provider. Task-level assessment solves this:

| Level | Who Assesses | Accuracy |
|-------|--------------|----------|
| Contract | Customer (non-expert) | Poor |
| Phase | Mixed | Medium |
| **Task** | **Provider (expert)** | **High** |

### Who Assigns What

| Field | Who Assigns | When |
|-------|-------------|------|
| Task **weight** (w) | Customer | Planning phase — reflects task importance |
| Task **stake** (s) | Derived | Computed from weight via first equation |
| Task **difficulty** (d) | Provider | At contract acceptance |

### The Process

1. **Planning phase** (customer's responsibility): Customer defines tasks and assigns weights (stake percentages)
2. **At acceptance**: Provider reviews tasks, assesses difficulty for each
3. **Provider can request refinement**: If tasks are too vague or large, provider can request breakdown before committing
4. **Phase difficulty emerges**: Aggregated from task difficulties using stake-weighted average

### Bidirectional Incentive for Accuracy

Both parties' future trust depends on getting difficulty right:
...
| **Customer** | Project fails or drags | Commitment trust decreases; scope stability suffers |

**Best to get it right at acceptance.** Neither party benefits from optimistic or inflated ratings—the outcome reveals the truth.

### Example

A $10,000 Implementation phase. Customer assigned weights during Planning; provider assesses difficulty at acceptance:

| Task | Weight | Stake | Difficulty | Provider |
|------|--------|-------|------------|----------|
| T1: Database schema | 10% | $1,000 | 3 | Alice |
| T2: OAuth2 integration | 25% | $2,500 | 7 | Alice |
| T3: Session management | 20% | $2,000 | 5 | Bob |
| T4: Rate limiting | 15% | $1,500 | 4 | Bob |
| T5: Security audit | 30% | $3,000 | 6 | Carol |

**Phase difficulty** = (3×1000 + 7×2500 + 5×2000 + 4×1500 + 6×3000) / 10000 = **5.35**

Each provider builds trust proportional to their task outcomes, not the whole phase.

Each task is a full contract (sub-subcontract), enabling individual trust attribution for team members.
